...
**Cập nhật lần cuối:** 2026-08-08 | **Bởi:** tech-lead | **Version:** 2.0
...
| Scripts | `scripts/` | Automation scripts hỗ trợ agent | `md_to_docx_kztek.py`, `link-global.ps1`, `review-package.sh`, `sync-to-gemini.py` + `git-hooks/post-commit` + `install-sync-hook.ps1` (CONFIRMED, 2026-08-06) |
...
| Agent Dashboard Backend | `tools/agent-dashboard/backend/` | FastAPI: file-watcher, WebSocket, SQLite ingestion, account mgmt, OAuth session management, usage metering | `main.py` (Sprint 5 BUG-004: broadcast `chain_updated` với `parent_session_id` khi child event; include `pipeline_router`), `state_manager.py`, `db/` (2026-08-08 refactor: tách `db.py` 1598 dòng → package 6 module — `schema.py` migrations+init, `sessions.py` CRUD+shaper+list/history, `chain.py` get_session_chain+dispatcher history + `_sanitize_text()` (2026-08-08 encoding fix: làm sạch lone surrogates từ errors='surrogateescape' trong payload_json cũ) + `_extract_user_turn_text()` gọi `_sanitize_text` trước khi trả về, `aggregate.py` get_pipeline_aggregate+token summary, `events.py` write path, `cursors.py`. `__init__.py` re-export → backward compatible), `parser.py` (Sprint 3: ai-title → `is_meta=True`; early-return for no-timestamp BUG-003; `first_user_text` extraction), `config.py` (Sprint 3: `MODEL_CONTEXT_WINDOW` dict + `resolve_max_context(model)`), `models.py` (Sprint 3: `ParsedLine` + `ai_title`, `first_user_text`, `is_meta` fields), `watcher.py`, `tail_reader.py` (CONFIRMED: đã dùng `open(path, "rb")` + `decode("utf-8", errors="replace")` từ đầu — Windows-safe, không dùng locale cp1252), `scripts/reingest_recent_sessions.py` (NEW 2026-08-08: one-time script re-read JSONL với encoding="utf-8" explicit, UPDATE payload_json cho sessions gần đây), `routes/accounts.py` (Sprint 5: `GET /usage/active` + `GET /{acc_id}/usage` — Bearer token, httpx, oauth-only; Sprint 6: _broadcast_account_change sends kind and key/oauth_masked; BUG-002: POST returns 409 on ACCOUNT_NAME_DUPLICATE for both api_key and oauth_session kinds), `routes/sessions.py` (Sprint 3: `GET /api/sessions/{id}/chain`), `routes/pipeline.py` (NEW Sprint 5 FR-005: `GET /api/pipeline/aggregate?project=&window=`; Sprint 6 FR-006: support group_by query param), `usage_service.py` (NEW Sprint 5: `UsageInfo`, `get_usage()`, `_pct()`, in-memory cache 60s TTL, httpx 5s timeout), `accounts.py` (v2 — kind discriminator, OAuth snapshot; BUG-002: `_name_exists()` + duplicate-name guard in `add_account`/`add_oauth_account` → raises `ValueError("ACCOUNT_NAME_DUPLICATE")`), `oauth_service.py` (activate_oauth_account requires `refresh_lock: asyncio.Lock`, auto-refresh scheduler, subprocess invoke) |
...
|------|------------|------|------------|-------|
| 2026-08-08 | `tools/agent-dashboard/backend/agent_dashboard/db/` | Refactor | Tách `db.py` (1598 dòng) → package `db/` gồm 6 module: `schema.py` (migrations+init), `cursors.py`, `sessions.py` (CRUD+shaper+list/history/detail/by-project), `events.py` (write path), `chain.py` (`get_session_chain`+`_backfill_chain_results`+dispatcher history), `aggregate.py` (`get_pipeline_aggregate`+`get_token_summary`). `__init__.py` re-export mọi symbol public — tất cả caller `from .. import db as db_module` + test patch `agent_dashboard.db.X` vẫn hoạt động; 266/266 test pass, không đổi behavior | tech-lead |
