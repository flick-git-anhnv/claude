...
| Q4 | **F-07 Account Switcher hiện chỉ hỗ trợ tài khoản có API key (`ANTHROPIC_API_KEY`). Người dùng Claude Pro/Max (OAuth login, không có API key tĩnh) KHÔNG dùng được tính năng này.** | High (gap phát hiện sau khi sprint 1 đã build xong) | **Backlog sprint 2:** mở rộng Account Manager hỗ trợ thêm loại credential "OAuth Session" — lưu/switch giữa các session-token đã đăng nhập qua `claude login`, thay vì chỉ API key tĩnh. Cần Tech Lead điều chỉnh TDD (data model + UI) trước khi Senior/Junior Dev code. Quyết định user (2026-08-06): triển khai SAU khi sprint 1 (API key only) đã qua QA/Deploy — không block release hiện tại. |
| FR-002 | **Hiển thị % context window đã dùng / còn lại** cho mỗi session — dùng công thức đã xác minh: `(input_tokens + cache_creation_input_tokens + cache_read_input_tokens)` của **lượt gọi gần nhất** (KHÔNG cộng dồn cả session) ÷ `max_input_tokens` (lấy động qua `GET /v1/models/{id}`, không hardcode vì đổi theo model — Sonnet 5/Opus 5/Fable 5 = 1M, Haiku 4.5 = 200K). | Medium — cần thêm field lưu "usage của dòng log gần nhất" riêng biệt với tổng cộng dồn hiện có | **Backlog Sprint 3.** Parser cần lưu thêm snapshot usage của dòng cuối cùng (không chỉ cộng dồn), cache `max_input_tokens` per-model (gọi Models API 1 lần, cache lại — không gọi mỗi request). |
| FR-003 | **Thay session ID thô bằng "tên" dễ đọc** cho các session không gọi Agent con (không có subagent_type/description) — hiện tại fallback hiển thị "— session 79979155-b132-433...". | Low — cần xác minh field nguồn trước khi code | **Backlog Sprint 3.** Đề xuất: dùng nội dung tin nhắn đầu tiên của user trong session (truncate) làm tên hiển thị, tương tự cách Claude Code tự đặt tên session. CẦN xác minh field thật trong `.jsonl` trước khi code (không đoán) — kiểm tra xem log có sẵn `summary`/title field hay phải tự trích từ user message đầu tiên. |
| Q4b | **Bản "đầy đủ" cho OAuth Session (quyết định user 2026-08-06):** Không chỉ refresh tài khoản đang active (passive, giống Claude Code CLI mặc định) — mà chủ động background job tự refresh **TẤT CẢ** account OAuth đã lưu (kể cả không active), để khi user switch qua account nào cũng dùng ngay không cần đăng nhập lại. | High — cần R&D trước khi TDD sprint 2, có rủi ro kỹ thuật chưa xác nhận | **Cần Tech Lead/Senior Developer research (R&D spike) trước khi TDD sprint 2**, làm rõ: (1) Anthropic có rotate refresh_token mỗi lần dùng không (nếu có → phải ghi đè token mới vào storage ngay sau mỗi lần refresh, tuyệt đối không refresh trùng 1 token 2 lần song song vì lần sau sẽ vô hiệu lần trước); (2) tần suất polling hợp lý (đề xuất: refresh khi access_token còn ~20% thời gian sống, không phải theo lịch cố định); (3) xử lý khi 1 account bị revoke/logout từ nơi khác — job phải phát hiện lỗi refresh và đánh dấu account đó "cần đăng nhập lại" thay vì retry vô hạn; (4) rủi ro bảo mật khi lưu nhiều refresh_token còn hiệu lực cùng lúc trên máy local — vẫn theo mức bảo mật đơn giản đã chốt (mã hoá nhẹ, không OS keychain) nhưng cần cảnh báo rõ trong Account Manager UI. Không tự phỏng đoán chi tiết endpoint/protocol nội bộ của Anthropic — phải verify thực tế bằng cách bắt log traffic của `claude login`/CLI trước khi code, không suy diễn. |
