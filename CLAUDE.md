...
Ví dụ hành động nhỏ hợp lệ:
- Cài 1 package còn thiếu
- Kiểm tra lại path bằng `ls` / `Glob`
- Đọc lại file config để xác nhận giá trị thực tế
...
**KHÔNG gợi ý compact khi:**
- Đang giữa bước (chưa commit, còn file đang sửa dở)
- Bước tiếp theo ngắn (ước tính ≤ 5 tool calls)
- User vừa mới chạy /compact trong vòng 2 bước vừa rồi
