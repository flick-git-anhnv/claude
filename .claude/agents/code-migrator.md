...
- [ ] **Đọc source gốc & lập bảng inventory chi tiết (§2A) — BẮT BUỘC trước khi viết bất kỳ dòng code đích nào.** Không đoán hành vi.
...
### G6 — Verify & QA
- [ ] **(Avalonia — CỔNG HOÀN THÀNH BẮT BUỘC) Build test thực tế trên CẢ Windows VÀ Linux** sau khi migration xong. Cả hai phải build thành công 0 lỗi thì migration mới được coi là DONE:
  ```bash
  # Trên Windows
  dotnet build <solution> -c Release -r win-x64        # → phải 0 lỗi
  # Trên Linux/Ubuntu (máy Linux, WSL, hoặc Docker mcr.microsoft.com/dotnet/sdk:8.0)
  dotnet build <solution> -c Release -r linux-x64      # → phải 0 lỗi
  dotnet publish <project> -c Release -r linux-x64 --self-contained   # xác nhận runtime asset đủ
  ```
  - [ ] **Lưu log build cả 2 nền tảng** làm bằng chứng (`docs/devops/[migration-slug]/BUILD-win-x64.log` + `BUILD-linux-x64.log`).
  - [ ] Nếu không có máy Linux thật → build trong **Docker .NET SDK** hoặc **WSL**; ghi rõ môi trường đã test trong ADR.
  - [ ] Linux build lỗi → quay lại §3.3 rà dependency Windows-only; **KHÔNG** đánh dấu DONE.
- [ ] QA Engineer smoke test path chính (startup, luồng nghiệp vụ cốt lõi) — tối thiểu trên Windows; trên Linux nếu hạ tầng cho phép.
- [ ] Đối chiếu behavior parity với bản nguồn.
- [ ] QA Lead sign-off (P0/P1 phải sạch + build 2 nền tảng xanh).
...
Với **mỗi** project ✅/⚠️, lập bảng theo loại thành phần:
...
# (kiểm tra publish Linux nếu cần chắc chắn runtime asset đủ)
...
- **(Avalonia)** Build trên Windows xanh nhưng **Linux lỗi** → BLOCK, quay lại §3.3, không được coi là hoàn thành.
...
- `docs/architecture/[migration-slug]/ADR-*.md` — chiến lược migration + **bảng inventory chi tiết** (§2A, 2 cấp) + **3 bảng mapping** (§2 G2).
...
- **(Avalonia)** Log build **cả 2 nền tảng** xanh: `docs/devops/[migration-slug]/BUILD-win-x64.log` + `BUILD-linux-x64.log` (G6).
...
### Build: [✅ 0 lỗi]   ### Component tái dùng: [list]   ### Build mới: [list]
